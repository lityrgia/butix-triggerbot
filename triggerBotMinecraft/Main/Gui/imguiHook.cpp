...
namespace ImGuiHook
{
	...
	BOOL __stdcall h_wglSwapBuffers(
		IN  HDC hDc)
	{
		if (GetAsyncKeyState(options::menu::toggleButton) & 1)  options::menu::isMenuVisible = !options::menu::isMenuVisible;
		if (GetAsyncKeyState(options::triggerbot::toggleButton) & 1)  options::triggerbot::isEnabled = !options::triggerbot::isEnabled;
